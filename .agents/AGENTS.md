...
### 3.8 맵 청크 카메라 바운더리 및 트리거 규칙
- **바운더리 및 트리거 필수 적용**: 새로운 맵 청크 프리팹(`MapChunk`)을 생성할 때, 반드시 플레이어 진입을 감지하는 `MapChunkBoundaryTrigger`가 포함되어야 하고 `cameraBoundaryCollider` 필드가 유효한 `Collider2D`로 지정되어 있어야 합니다.
- **화면 튐(Snap) 방지**: 런타임에 카메라 바운더리가 교체될 때 카메라가 휙휙 튀는 현상을 방지하기 위해 다음 두 가지 방안 중 하나를 준수하여 설계해야 합니다.
  - *방안 A (추천)*: 듀얼 Cinemachine 카메라를 적용하여, 플레이어가 트리거를 밟을 때 비활성 카메라의 바운더리를 교체한 후 우선순위를 높여 Cinemachine 고유의 부드러운 Blend(예: 0.5초 Ease In Out)를 이용해 전환합니다.
  - *방안 B*: 각 청크의 `cameraBoundaryCollider`를 인접 청크 영역과 약 3~5 unit 가량 오버랩(Overlap)되도록 좌우로 넓게 설정하고, `MapChunkBoundaryTrigger`는 청크 시작 지점이 아닌 플레이어가 청크 안으로 확실히 진입한 깊은 지점에 배치하여 교체 순간의 튐을 억제합니다.

...
이 프로젝트는 Unity 엔진 기반의 2D 횡스크롤/액션 플랫포머 게임입니다.
  ...
  - 모든 페이즈 왼쪽에서 오른쪽으로 움직임.
    - 1분이 지나갈 때 마다 촉수함정의 주기가 빨라지거나 새로운 몬스터가 증가하는 식
   상자 : 일반 타일 중에 랜덤으로 스폰
...
- **화면 튐(Snap) 방지**: 런타임에 카메라 바운더리가 교체될 때 카메라가 휙휙 튀는 현상을 방지하기 위해 다음 두 가지 방안 중 하나를 준수하여 설계해야 합니다.
  - *방안*: 듀얼 Cinemachine 카메라를 적용하여, 플레이어가 트리거를 밟을 때 비활성 카메라의 바운더리를 교체한 후 우선순위를 높여 Cinemachine 고유의 부드러운 Blend(예: 0.5초 Ease In Out)를 이용해 전환합니다.
---
